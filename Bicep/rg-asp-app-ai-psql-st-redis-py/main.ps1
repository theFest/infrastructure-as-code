Function rg-asp-app-ai-psql-st-redis-py {
  <#
  .SYNOPSIS
  Python / Linux
  ...
  .EXAMPLE
  rg-asp-app-ai-psql-st-redis-py
  rg-asp-app-ai-psql-st-redis-py -any_of_params_that_you_wanna_customize
  ...
  param(
    ...
    [Parameter(Mandatory = $true, HelpMessage = "Define Redis Host")]
    [string]$RedisHost = "REDIS_HOST",

    [Parameter(Mandatory = $true, HelpMessage = "Define a Redis password")]
    [string]$RedisPass = "REDIS_PASSWORD",

    [Parameter(Mandatory = $false, HelpMessage = "Define a Redis port")]
    [string]$RedisPort = "REDIS_PORT"
  )
  BEGIN {
    ## Check and set script path
    Push-Location $PSScriptRoot
    Write-Host CurrentDirectory $PWD
